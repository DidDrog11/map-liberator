...
Map Liberator is open-source and released under the MIT License. The source code is available on GitHub at [Insert GitHub Link]. The specific version used for this manuscript is archived on Zenodo (DOI: [Insert Zenodo DOI]). A hosted web application for demonstration is available at [Insert ShinyApps Link]. The Lassa fever datasets generated and analysed during the current study are available within the GitHub repository.
...
Simons, D conceptualised the tool, wrote the software code, conducted the Lassa fever case study validation, and authored the manuscript. 
